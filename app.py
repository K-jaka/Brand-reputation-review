...
# 3. Data Loading
# FIX: ttl=3600 means cache expires after 1 hour — picks up new CSV data automatically
...
# The scraper stores titles as e.g. "Review cat-ear-beanie-5"
# This function strips "Review", replaces dashes, and removes the trailing number
def clean_product_name(name):
    ...
    # 2. Remove the first word — scraper prefixes every title with "Review"
    parts = name.split()
    if len(parts) > 1:
        name = " ".join(parts[1:])

    # 3. Remove trailing number if the name ends with one
...
try:
    ...
    st.error("'scraped_reviews.csv' not found. Please run scraper.py first!")
    st.stop()
...
# Section: Testimonials (Page 2)
    ...
    # FIX: show 10 most recent reviews instead of first 10 rows
    testi_df = df[['text']].tail(10).rename(columns={'text': 'Customer reviews'})
...
# Section: Reviews (Page 3)
    ...
    if not filtered_df.empty:
        with st.spinner(f"Analyzing sentiment for {selected_month_name}..."):
            # FIX: truncate reviews to 512 chars — DistilBERT's token limit
            texts = [str(t)[:512] for t in filtered_df['text'].tolist()]

            # FIX: process in batches of 32 to avoid memory issues
            # FIX: wrapped in try/except for a friendly error message on failure
            try:
